...
warfarin_combined_error() = lowercase(get(ENV, "WARFARIN_JULIA_RESIDUAL_MODEL", "additive")) == "combined"

function warfarin_variance(pred, sigma_add, sigma_prop)
    sigma_add * sigma_add + (sigma_prop * pred) * (sigma_prop * pred)
end
...
function h_i(subj::SubjectData, x, eta, representation::Symbol; dt::Float64=0.25)
    ...
    sigma_prop_pk = combined ? exp(x[10]) : zero(sigma_pk)
    sigma_prop_pd = combined ? exp(x[11]) : zero(sigma_pd)
    ...
    for (y, pred) in zip(subj.pk_obs, pk_hat)
        v = warfarin_variance(pred, sigma_pk, sigma_prop_pk)
        total += 0.5 * log(v) + 0.5 * (y - pred)^2 / v
    end

    pd_hat = predict_pd(subj, x, eta, representation; dt=dt)
    for (y, pred) in zip(subj.pd_obs, pd_hat)
        v = warfarin_variance(pred, sigma_pd, sigma_prop_pd)
        total += 0.5 * log(v) + 0.5 * (y - pred)^2 / v
    end
